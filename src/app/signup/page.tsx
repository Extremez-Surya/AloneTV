...
export default function SignUpPage() {
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isNetworkError) {
        console.warn('Supabase offline. Registering via Demo Mode...');
        setIsDemoMode(true);
        localStorage.setItem('alonetv_user', JSON.stringify({
          email: email || 'demo@example.com',
          name: name || 'Demo Watcher',
          demo: true
        }));
        setSuccess(true);
        setTimeout(() => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-bg-primary px-4 py-12 pt-[72px]">
      <div className="w-full max-w-md">
        ...
        <div className="bg-bg-card rounded-2xl p-8 border border-border shadow-level-3">
          <h1 className="text-xl font-semibold text-text-primary text-center mb-1">Create Account.</h1>
          <p className="text-sm text-text-muted text-center mb-6">Sign up to begin streaming</p>
          ...
          {success ? (
            <div className="text-center py-4 space-y-4">
              ...
              <p className="text-xs text-text-muted leading-relaxed">
                {isDemoMode 
                  ? 'Registration successful! Auto-logging in via Demo session...' 
                  : `We sent a validation link to ${email}. Verify it to start watching.`}
              </p>
              {isDemoMode && (
            ...
            <form onSubmit={handleSignUp} className="space-y-4">
              <div>
                <label htmlFor="name" className="block text-xs font-semibold uppercase tracking-wider text-text-muted mb-2 font-mono">
                  Display Name
                </label>
                <input
                  id="name"
                  ...
                  required
                  className="w-full px-3.5 py-2 bg-bg-secondary border border-border rounded-lg text-sm text-text-primary placeholder-text-muted focus:outline-none focus:border-accent-purple transition-colors"
                />
              </div>
              ...
              <div>
                <label htmlFor="password" className="block text-xs font-semibold uppercase tracking-wider text-text-muted mb-2 font-mono">
                  Password
                </label>
                <input
                  id="password"
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full py-2.5 bg-text-primary text-bg-card text-sm font-semibold rounded-lg hover:bg-black/95 transition-all disabled:opacity-50 mt-2"
              >
              ...
              <button
                type="button"
                onClick={handleDirectDemoRegister}
        ...
        <p className="text-center text-text-muted mt-6 text-xs font-medium">
          ...
          <Link href="/signin" className="text-accent-purple hover:underline">
            Sign in
          </Link>
        </p>
      </div>
